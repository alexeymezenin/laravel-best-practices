...
[Храните даты в стандартном формате. Используйте читатели (accessors) и преобразовател (mutators)и для преобразования формата](#Храните-даты-в-стандартном-формате-Используйте-читатели-и-преобразователи-для-преобразования-формата)
...
Внедрение классов через синтаксис `new Class` создает сильную связанность между частями приложения и усложняет тестирование. Используйте контейнер или фасады.
...
Передайте данные из файла `.env` в конфигурационные файлы и используйте `config()` в приложении, чтобы обращаться к этим данным.
...
### **Храните даты в стандартном формате. Используйте читатели (accessors) и преобразователи (mutators) для преобразования формата**
...
Старайтесь не использовать «сырой» PHP-код в шаблонах Blade.
